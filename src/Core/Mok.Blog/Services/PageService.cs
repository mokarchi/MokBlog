...
namespace Mok.Blog.Services
{
    public class PageService : IPageService, INavProvider
    {
        ...
        /// source, throws <see cref="MokException"/> if not found, the returned post is tracked.
        /// </summary>
        /// <param name="id">A <see cref="Page"/> id.</param>
        /// <returns></returns>
        /// <exception cref="MokException">Thrown if post is not found.</exception>
        ...
        /// <summary>
        /// Ensures <see cref="Page"/> title is valid and throws <see cref="MokException"/> if validation fails.
        /// </summary>
        /// <param name="page">A <see cref="Page"/> to be created or updated.</param>
        /// <exception cref="MokException">Throws when validation fails</exception>
        /// <remarks>
        ...
        internal async Task EnsurePageTitleAsync(Page page)
        {
            ...
            if (page.IsParent)
            {
                ...
                {
                    throw new MokException(DUPLICATE_TITLE_MSG);
                }
            }
        ...
        /// <summary>
        /// Ensures <see cref="Page"/> slug is valid, throws <see cref="MokException"/> if the 
        ...
        internal async Task EnsurePageSlugAsync(string slug, Page page)
        {
            ...
            if (page.IsParent)
            {
                ...
                {
                    throw new MokException(EExceptionType.DuplicateRecord, DUPLICATE_SLUG_MSG);
                }

                // parent needs to check reserved slugs
                if (Reserved_Slugs.Contains(slug))
                {
                    throw new MokException(EExceptionType.DuplicateRecord, string.Format(RESERVED_SLUG_MSG, slug));
                }
            }
            else
            {

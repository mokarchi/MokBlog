...
namespace Mok.Blog.Tests.Services
{
    ...
    public class TagServiceTest
    {
        ...
        /// </summary>
        [Fact]
        public async void CreateTag_Throws_MokException_If_Title_Already_Exist()
        {
            // Arrange: a category with a title that exists
            var tag = new Tag { Title = "Technology" };

            // Act and Assert: error message
            var ex = await Assert.ThrowsAsync<MokException>(() => tagService.CreateAsync(tag));
        ...
        /// </summary>
        [Fact]
        public async void GetTagBySlug_Throws_MokException_If_Not_Found()
        {
            await Assert.ThrowsAsync<MokException>(() => tagService.GetAsync(100));
            await Assert.ThrowsAsync<MokException>(() => tagService.GetBySlugAsync("slug-not-exist"));
        }
